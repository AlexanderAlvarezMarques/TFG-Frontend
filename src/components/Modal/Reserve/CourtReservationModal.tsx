...
export const CourtReservationModal = ({onClose, courtId, courtNumber, date, hour, minute}: Props) => {
    ...
    const submitForm = async (e: any) => {
        e.preventDefault();
        const response = ReserveService.createReserve(courtId, date, Number(hour), Number(minute), Number(e.target.duration.value));

        response.then((res) => {
            if (res.status === 201) {
                setSuccess(true);
                setReserveId(res.data.id);
            }
        }).catch((error) => {
            setError(true);
